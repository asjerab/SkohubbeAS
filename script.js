...
const shoeListWrapper = document.getElementById('shoe-list-wrapper');

// Display the filtered shoes
function filterShoes(searchText) {
    const filteredShoes = products.filter(shoe =>
        shoe.navn.toLowerCase().includes(searchText.toLowerCase())
    );
...
function displayShoes(shoes) {
    ...
    if (shoes.length > 0) {
        shoes.forEach(shoe => {
            ...
            shoeItem.classList.add('shoe_item');
            shoeItem.innerHTML = `
          <img src="images/${shoe.navn}.png" alt="${shoe.navn}">
          <a href="${new URL('produkt.html', window.location.origin)}?id=${shoe.id}">${shoe.navn}</a>
          <p>Price: $${shoe.pris}</p>
          <p>Color: ${shoe.farge}</p>
      `;
            shoeList.appendChild(shoeItem);
        });
        shoeListWrapper.classList.add('show');
    } else {
        shoeListWrapper.classList.remove('show');
    }
}
